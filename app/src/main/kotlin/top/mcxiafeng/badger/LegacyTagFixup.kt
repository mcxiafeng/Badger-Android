...
class LegacyTagFixup(
    ...
    suspend fun runOnce() {
        try {
            ...
            if (pending.isEmpty()) {
                Log.d(TAG, "runOnce: 无遗留待补")
                return
            }
            Log.d(TAG, "runOnce: 开始重算 ${pending.size} 个 legacy tag 的 pinyinInitial")
            var success = 0
            for (tag in pending) {
                try {
                    ...
                    Log.w(TAG, "runOnce: 重算失败 id=${tag.id} name='${tag.name}'", e)
                }
            }
            Log.d(TAG, "runOnce: 完成,成功 $success/${pending.size}")
        } catch (e: Exception) {
            Log.w(TAG, "runOnce: 失败(可忽略,不影响主流程)", e)
        }
    }
