...
object AiOcrConfig {
    ...
    fun isConfigured(@Suppress("UNUSED_PARAMETER") ctx: android.content.Context): Boolean =
        PrefsStore.readBoolean(KEY_ENABLED, false)

    /** Alias kept for legacy callers — same predicate as [isConfigured]. */
    fun isAiOcrEnabled(ctx: android.content.Context): Boolean = isConfigured(ctx)

    fun setEnabled(@Suppress("UNUSED_PARAMETER") ctx: android.content.Context, b: Boolean) {
     ...
     */
    fun hasVisionModel(@Suppress("UNUSED_PARAMETER") ctx: android.content.Context): Boolean = true

    /** Alias for [hasVisionModel] used by some UI call-sites. */
    fun supportsVision(ctx: android.content.Context): Boolean = hasVisionModel(ctx)

    /** Returns the human-readable name of the model currently in use. */
    fun getModel(@Suppress("UNUSED_PARAMETER") ctx: android.content.Context): String =
        PrefsStore.readString(KEY_MODEL) ?: "qwen-vl"

    fun setModel(@Suppress("UNUSED_PARAMETER") ctx: android.content.Context, v: String) {
    ...
    fun isAiTagPrivacyAgreed(@Suppress("UNUSED_PARAMETER") ctx: android.content.Context): Boolean =
        PrefsStore.readBoolean(KEY_TAG_PRIVACY, false)

    fun setAiTagPrivacyAgreed(@Suppress("UNUSED_PARAMETER") ctx: android.content.Context, b: Boolean) {
        PrefsStore.writeBoolean(KEY_TAG_PRIVACY, b)
    }

    /** Alias for [setAiTagPrivacyAgreed] used by [ScannerSubDialogs]. */
    fun setPrivacyAgreed(ctx: android.content.Context, b: Boolean) = setAiTagPrivacyAgreed(ctx, b)
}
