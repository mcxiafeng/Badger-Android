        ...
        return try {
            ...
            when (e) {
                is java.net.ConnectException -> {
                    ...
                    BadgerLog.w(TAG, "[$tag] login ConnectException: msg=${e.message} reason=$reason", e)
                }
                is java.net.SocketTimeoutException -> {
                    BadgerLog.w(TAG, "[$tag] login SocketTimeoutException: msg=${e.message}", e)
                }
                is java.net.UnknownHostException -> {
                    BadgerLog.w(TAG, "[$tag] login UnknownHostException: msg=${e.message}", e)
                }
                is java.io.IOException -> {
                    ...
                    BadgerLog.w(TAG, "[$tag] login IOException chain:$chain", e)
                }
                is ApiException -> {
                    BadgerLog.w(TAG, "[$tag] login failed: code=${e.status} what=${e.what}")
                }
                else -> {
                    BadgerLog.w(TAG, "[$tag] login unexpected error: ${e.javaClass.simpleName}: ${e.message}", e)
                }
            }
            throw e
        }
