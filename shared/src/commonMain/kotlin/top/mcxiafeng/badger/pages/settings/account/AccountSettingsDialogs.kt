    ...
    BadgerDialog(
        ...
        onPositive = {
            ...
            if (looksLikeCredential) {
                BadgerLog.w(TAG, "EditServerUrlDialog: rejected url input that looks like credential (len=${input.length})")
                return@BadgerDialog
            }
            ...
            val cleaned = if (firstSlash > 0) {
                BadgerLog.w(TAG, "EditServerUrlDialog: url had path suffix, stripping (inLen=${input.length})")
            ...
            if (cleaned.startsWith("http://") || cleaned.startsWith("https://")) {
                if (hostPart.isBlank()) {
                    BadgerLog.w(TAG, "EditServerUrlDialog: rejected empty host (inLen=${input.length})")
                    return@BadgerDialog
                }
            }
            onConfirm(cleaned)
        ...
        top.yukonga.miuix.kmp.basic.TextButton(
            ...
            onClick = {
                ...
                BadgerLog.d(TAG, "EditServerUrlDialog: restore default URL")
            },
