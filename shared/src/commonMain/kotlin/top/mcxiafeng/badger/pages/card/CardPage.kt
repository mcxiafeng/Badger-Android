...
fun CardScreen(
    ...
    val exportFileLauncher = rememberDocumentSaveLauncher(
        ...
        suggestedName = "badger_collections.json",
    ) { ok ->
        scope.launch {
            if (ok) showToast("导出成功") else showToast("导出失败")
        }
    }
    val importFileLauncher = rememberDocumentPickLauncher("application/json") { bytes ->
        if (bytes != null) {
            scope.launch {
                try {
                    ...
                    BadgerLog.d(TAG, "importFromFile: ${conflicts.size} collections, ${conflicts.sumOf { it.contactConflicts.size }} contacts")
                } catch (e: Exception) {
                    BadgerLog.e(TAG, "importFromFile: failed", e)
                ...
                TopAppBar(
                    ...
                    actions = {
                        IconButton(onClick = {
                            ...
                            BadgerLog.d(TAG, "toggleSelectAll: isAllSelected=$isAllSelected")
                        }) {
                            Icon(
                        ...
                        CardOverflowMenu(
                            ...
                            onExport = {
                                ...
                                actualExportIds = ids
                                BadgerLog.d(TAG, "exportCollections: ids=${ids.size}")
                                scope.launch {
                                    pendingExportJson = onExportCollections(ids)
                                    exportFileLauncher.launch(pendingExportJson ?: "")
                                }
                            },
                                ...
                                onClick = {
                                    ...
                                    scope.launch {
                                        try {
                                            ...
                                            val fileName = "badger_share_${nowMs()}.json"
