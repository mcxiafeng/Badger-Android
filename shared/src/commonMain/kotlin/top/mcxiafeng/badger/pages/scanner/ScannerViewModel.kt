...
import top.mcxiafeng.badger.platform.QrEngineBootstrap
...
class ScannerViewModel : ViewModel() {
    ...
    suspend fun confirmScan(
        ...
        BadgerLog.d(TAG, "confirmScan: items=${selectedItems.size} existing=${existingContact?.id} source=$sourceType")
        ...
        val result: CommitResult = if (existingContact != null) {
            ...
            for ((_, info) in selectedItems) {
                ...
                if (last is CommitResult.Written) {
                    ...
                    BadgerLog.e(TAG, "confirmScan: save failed $last")
                    return last
                }
            }
            last
    ...
    suspend fun attachScan(
        ...
        BadgerLog.d(TAG, "attachScan: contactId=${contact.id} platforms=${info.platforms.keys}")
        val result = if (info.platforms.isNotEmpty() || info.phone != null || info.email != null) {
            ...
            contactRepository.updateContact(fresh.copy(updateTime = nowMs()))
    ...
    private suspend fun applyMarker(contactIds: List<Long>, markerConfig: ScanMarkerConfig) {
        ...
        contactIds.forEach { cid ->
            try {
                ...
                BadgerLog.d(TAG, "applyMarker: tagId=${markerConfig.tagId} -> contactId=$cid")
            } catch (e: Exception) {
                BadgerLog.e(TAG, "applyMarker failed cid=$cid", e)
            }
        }
    }
            ...
            viewModelScope.launch {
                try {
                    ...
                    if (bio.isNullOrBlank()) {
                        BadgerLog.d(TAG, "后台 AI 打标跳过: contactId=$cid 无 bio")
                        return@launch
                    }
                    ...
                    val candidates = try {
                        ...
                        BadgerLog.w(TAG, "后台 AI 失败,降级 fallbackLocal: cid=$cid, ${e.message}")
                    ...
                    BadgerLog.d(TAG, "后台 AI 打标完成: contactId=$cid, candidates=${candidates.size}")
                } catch (e: Exception) {
                    BadgerLog.e(TAG, "后台 AI 打标失败: contactId=$cid", e)
                }
            }
        }
    }
    ...
    companion object {
        ...
        private val _openCvInitialized = kotlinx.coroutines.flow.MutableStateFlow(false)
        internal val openCvInitialized: kotlinx.coroutines.flow.StateFlow<Boolean> = _openCvInitialized
    }

    private suspend fun ensureOpenCvInitialized() {
        QrEngineBootstrap.ensureReady()
        _openCvInitialized.value = true
    }
}
