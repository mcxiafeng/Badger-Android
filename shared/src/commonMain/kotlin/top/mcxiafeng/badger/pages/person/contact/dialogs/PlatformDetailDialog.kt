    ...
    if (show) WindowDialog(
        ...
        Column(
            ...
            if (!detailDisplayName.isNullOrBlank()) {
                ...
                DetailInfoRow(label = "昵称", value = detailDisplayName)
            }
            val detailValue = entry.value
            if (!detailValue.isNullOrBlank()) {
                Spacer(modifier = Modifier.height(8.dp))
                DetailInfoRow(label = "ID", value = detailValue)
            }
            Spacer(modifier = Modifier.height(8.dp))
            DetailInfoRow(label = "主页链接", value = entry.jumpLink)
            val detailOriginalLink = entry.originalLink
            if (!detailOriginalLink.isNullOrBlank() && detailOriginalLink != entry.jumpLink) {
                Spacer(modifier = Modifier.height(8.dp))
                DetailInfoRow(label = "原始链接", value = detailOriginalLink)
            }
    ...
    Column(
        modifier = Modifier
            ...
            .combinedClickable(
                ...
                onClick = {},
                onLongClick = {
                    Methods.copyToClipboard(label, value)
                    showToast("已复制 $label")
                },
            )
    ) {
