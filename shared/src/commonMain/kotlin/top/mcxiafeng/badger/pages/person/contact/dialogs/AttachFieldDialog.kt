...
internal suspend fun attachCurrentContactToExisting(
    ...
    if (!sourceAvatarPath.isNullOrBlank()
        ...
        val sourceBytes = ImageFiles.loadImageBytes(sourceAvatarPath)
        if (sourceBytes != null) {
            val destPath = ImageFiles.saveAvatarImage(sourceBytes, "contact_${freshExisting.id}_avatar.webp")
            if (destPath != null) {
                repository.updateContact(
                    freshExisting.copy(avatarPath = destPath, updateTime = nowMs())
                )
                avatarAttached = true
            }
        }
