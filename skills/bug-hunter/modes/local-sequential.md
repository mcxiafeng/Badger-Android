...
1. Read `SKILL_DIR/skills/recon/SKILL.md` from disk — do NOT act from memory.
...
2. **If `.bug-hunter/triage.json` exists:**
   ...
   - Write your Recon output to `.bug-hunter/recon.json`.
...
3. **If `.bug-hunter/triage.json` does NOT exist** (fallback — Recon called directly):
   ...
   - Write complete output to `.bug-hunter/recon.json`.
...
1. Read `SKILL_DIR/skills/hunter/SKILL.md` from disk.
2. Read `SKILL_DIR/skills/doc-lookup/SKILL.md` from disk.
...
4. Execute the Hunter instructions yourself:
   ...
   - Read each file directly. Do NOT rely on memory from earlier phases.
   - Apply the mandatory security checklist sweep in the Hunter skill on every CRITICAL and HIGH file.
   - Track which files you actually read — be honest about coverage.
   - For each bug found, record it in the exact schema specified by the Hunter skill.
5. Write your complete findings to `.bug-hunter/hunter-findings.json`.
6. Validate the artifact immediately:
   ```bash
   node "$SKILL_DIR/scripts/schema-validate.cjs" findings ".bug-hunter/hunter-findings.json"
   ```
...
1. Read `SKILL_DIR/skills/skeptic/SKILL.md` from disk.
2. Read `SKILL_DIR/skills/doc-lookup/SKILL.md` from disk.
...
4. Read `.bug-hunter/hunter-findings.json` to get the findings list.
5. For EACH finding:
   - Re-read the actual code at the reported file and line. This is MANDATORY
...
1. Read `SKILL_DIR/skills/referee/SKILL.md` from disk.
2. **Switch mindset**: you are the impartial Referee. You trust neither the Hunter nor the Skeptic.
3. Read both `.bug-hunter/hunter-findings.json` and `.bug-hunter/skeptic.json`.
4. For each finding:
   - **Tier 1 (all Critical + top 15 by severity):** Re-read the actual code
     yourself a THIRD time. Construct the runtime trigger independently. Make
     your own judgment.
...
| `.bug-hunter/hunter-findings.json` | B | All Hunter findings in canonical JSON |
...
- Do NOT claim "full coverage" or "audit complete" unless every queued
  scannable source file was actually read and has status DONE.
